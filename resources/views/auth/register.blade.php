<html lang="en" dir="ltr" class="h-full font-sans antialiased dark">
...
<head>
    ...
    <!-- Styles -->
    <link rel="stylesheet" href="/vendor/nova/app.css?id=6e65850ccef9787f521955e9d231a2b2">
...
<body class="min-w-site text-sm font-medium min-h-full text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-900">
    <div class="py-6 px-1 md:px-2 lg:px-6">
        <div class="mx-auto py-8 max-w-sm flex justify-center"><svg class="h-8 h-6" viewBox="0 0 204 37"
                xmlns="http://www.w3.org/2000/svg">
                <defs>
                    <radialGradient cx="-4.619%" cy="6.646%" fx="-4.619%" fy="6.646%" r="101.342%"
                        gradientTransform="matrix(.8299 .53351 -.5579 .79363 .03 .038)" id="a">
                ...
                <g fill-rule="nonzero" fill="none">
                    ...
                    <path
                        ...
                        class="fill-current text-gray-600 dark:text-white"></path>
                </g>
            </svg></div>
        <div>
            <form class="bg-white dark:bg-gray-800 shadow rounded-lg p-8 max-w-[25rem] mx-auto" method="POST"
                action="{{ route('register') }}">
                @csrf
                <h2 class="text-2xl text-center font-normal mb-6">Sign Up</h2><svg class="block mx-auto mb-6"
                    xmlns="http://www.w3.org/2000/svg" width="100" height="2" viewBox="0 0 100 2">
                    <path fill="#D8E3EC" d="M0 0h100v2H0z"></path>
                </svg>
                <div class="mb-6"><label class="block mb-2" for="name">Full Name</label><input
                        class="w-full form-control form-input form-control-bordered" id="name" type="text" name="name"
                        autofocus="" autocomplete="name" required=""></div>
                <div class="mb-6"><label class="block mb-2" for="email">Email Address</label><input
                        class="w-full form-control form-input form-control-bordered" id="email" type="email"
                        name="email" autofocus="" autocomplete="email" required=""></div>
                <div class="mb-6"><label class="block mb-2" for="password">Password</label><input
                        class="w-full form-control form-input form-control-bordered" id="password" type="password"
                        name="password" autocomplete="current-password" required=""></div>
                <div class="flex mb-6">

                </div><button type="submit"
                    class="border text-left  mb-5 appearance-none cursor-pointer rounded text-sm font-bold focus:outline-none focus:ring ring-primary-200 dark:ring-gray-600 relative disabled:cursor-not-allowed inline-flex items-center justify-center shadow h-9 px-3 bg-primary-500 border-primary-500 hover:[&amp;:not(:disabled)]:bg-primary-400 hover:[&amp;:not(:disabled)]:border-primary-400 text-white dark:text-gray-900 w-full flex justify-center"><span
                        class="flex items-center gap-1"><span>Save</span></span></button>
                <a href="{{route('login')}}"
                    class="border mb-3 text-left appearance-none cursor-pointer rounded text-sm font-bold focus:outline-none focus:ring ring-primary-200 dark:ring-gray-600 relative disabled:cursor-not-allowed inline-flex items-center justify-center shadow h-9 px-3 bg-primary-500 border-primary-500 hover:[&amp;:not(:disabled)]:bg-primary-400 hover:[&amp;:not(:disabled)]:border-primary-400 text-white dark:text-gray-900 w-full flex justify-center"><span
                        class="flex items-center gap-1"><span>Login</span></span></a>
            </form>
        </div>
    </div>
